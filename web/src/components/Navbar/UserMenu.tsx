  ...
  if (!user) {
    return (
      <div className="flex items-center space-x-3">
        <Link
          href="/login"
          className={`transition-colors duration-300 text-sm font-medium ${theme === "dark" ? "text-gray-300 hover:text-white" : "text-gray-600 hover:text-gray-900"}`}
        >
          Sign In
        </Link>
  ...
  return (
    <div className="relative" ref={userMenuRef}>
      <button
        ...
        className={`flex items-center space-x-2 transition-colors duration-300 group ${theme === "dark" ? "text-gray-300 hover:text-white" : "text-gray-600 hover:text-gray-900"}`}
      >
        {user?.avatar ? (
          <Image
            src={user.avatar}
            ...
            width={32}
            height={32}
            className="rounded-full ring-2 ring-transparent group-hover:ring-purple-500 transition-all duration-300"
          />
        ) : (
      ...
      {showUserMenu && (
        <div
          className={`absolute right-0 top-full mt-2 w-56 backdrop-blur-md rounded-xl shadow-2xl overflow-hidden ${
            theme === "dark" ? "bg-black/95 border border-white/20" : "bg-white/95 border border-gray-200/50"
          }`}
        >
          <div className="p-2">
