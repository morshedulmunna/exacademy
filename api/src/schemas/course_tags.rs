...
pub(super) async fn create_course_tags(db: &Database) -> Result<()> {
    let coll = ensure_collection(db, "course_tags", Some(course_tags_validator())).await?;
