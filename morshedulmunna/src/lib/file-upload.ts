...
export class FileUploader {
  ...
  private static readonly FILE_TYPES: Record<"VIDEO" | "PDF" | "DOCUMENT" | "IMAGE" | "AUDIO" | "OTHER", FileTypeConfig> = {
    VIDEO: {
  ...
  static async deleteFile(fileUrl: string): Promise<void> {
    try {
      // Support both leading-slash url (/uploads/...) and relative (uploads/...)
      const normalized = fileUrl.startsWith("/") ? fileUrl.slice(1) : fileUrl;
      const filePath = path.join(process.cwd(), "src", "public", normalized);
